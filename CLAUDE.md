...
- `TODO.md` — the **book-1** work tracker, arranged **in the order the work happens**, restructured 2026-07-31. **Three phases, and they are not a priority list — each removes constraints the next would otherwise work around.** **Phase 1, engine (nibli):** every prompt is ready to paste; some of what the book concedes is an engine limitation rather than a design choice, and conceding a fixable limitation is dishonest. **Phase 2, author-gated decisions:** collected in one place rather than scattered, because several chapters cannot be revised until the decision above them is ruled — the ruling *is* what the chapter says. **Phase 3, chapter passes, chapter 1 through 14 in order:** one at a time, read whole, fix, verify, commit, move on; the per-chapter bullets are a floor for that pass, not its scope, and a chapter with no bullets still gets a pass. Everything after phase 3 is cross-cutting (constitution work no chapter owns, harness, data, research brief, legacy harvest, book-2 hold) — work it when a chapter pass reaches into it. Delete a bullet when it fully lands; update it if only partly done. Don't work book-2 items.

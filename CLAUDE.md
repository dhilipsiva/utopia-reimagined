...
- `new-book-plans/` — planning material for book-1, plus the constitution `constitution.nibli`. `3-spine.md`'s stratification table and chapter order are **generated** — don't edit the block by hand and don't transcribe its counts elsewhere; it went stale twice that way. The generated `assertion-surface-audit.md` is governed by the reviewed `assertion-surface-contracts.json`; edit the contract, then regenerate with `7-assertion-surface.py`, never hand-edit the report. The generated `record-integrity-assurance-case.md` is governed by `record-integrity-assurance-case.json`; regenerate it with `8-record-integrity-assurance.py`, and never present its current red verdict as deployed assurance. The generated `record-integrity-red-team.md` is governed by `record-integrity-red-team.json`; regenerate it with `9-record-integrity-red-team.py`. Its passing scenarios expose selected current gaps and do not establish record assurance. The digest dependency is 7 → 8 → 9: an upstream reviewed-source change must be propagated before any report is regenerated. Check every generated artifact with its script's `--check` mode. Verify constitution claims with the release `nibli-pin --kb`, never `nibli-host` — its wasm predates the `derived_only` and `entitled` corpus entries and silently drops the entire rights floor and every conclusion-only gate while still answering queries.
- `verify.sh` — **the one check.** `./verify.sh` took **474.46 s on 2026-08-04** for 544 chapter/floor pins, 23 isolated record snapshots with 145 reviewed pins, one deliberately failing executable control, and the source counterfactuals; the runtime is a property of the suites, not the engine. **`--quick` took 2.02 s on the same measurement and checks the red-team contract and generated report but does not execute its snapshots, the chapter/floor pin suite, or the counterfactual fixtures**, so run the full suite after any constitution edit. **It rebuilds `nibli-pin` before generated checks and executable suites** — ~0.2 s incremental, prints the nibli commit, and shares one cached `--strata` measurement between the spine and assertion-surface generators, because the binary was once three days stale and this repo silently ran the old engine to a green result. Override with `NIBLI_PIN` to pin a specific binary, or `NIBLI_SRC` if the checkout is elsewhere. The suite runs the spine, assertion-surface, assurance-case, and bounded red-team checks, the evidence-vocabulary guard, jargon sweep, absence claims, `reward` guards, control-scope guard, chapter/floor pins with cross-file `:expect-pins` reconciliation, executable record snapshots, and source counterfactuals. It exits non-zero on the first failure and names the claim that stopped being true. Run it before every commit and prefer it to any check by hand — each structural check is negative-controlled.

...
- **The learning route is book-1's whole delivery story**, ruled 2026-08-02 — the
  provisioning fork, closed. The destination book demonstrates delivery once —
  `teaches` + `hears` -> `learn`, dormant, arrival evidence upstream of the head — and
  grows no further routes: a meal record, a care record and the rest are operational
  design, book-2's layer by the seam, each priced as the widening it is when built
  there. Chapter 8 says it in the book's own register: the routes for the rest are the
  building of them, and the building belongs to the book about building. Do not add
  delivery routes to this constitution; the one route is the argument, and more would
  be dormant machinery without new argument.

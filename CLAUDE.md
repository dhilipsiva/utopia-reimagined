...
- `TODO.md` — the **book-1** work tracker, arranged **in the order the work happens**, restructured 2026-07-31. **Three phases, reordered 2026-08-02 at the author's direction — author-gated decisions first, engine handoffs second, chapter passes third.** **Phase 1, author-gated decisions:** collected in one place rather than scattered, because several chapters cannot be revised until the decision above them is ruled — the ruling *is* what the chapter says; this section was once destroyed by tooling and restored, so treat its contents as the most expensive lines in the file. **Phase 2, engine handoffs (nibli):** currently empty; prompts are written as one session addressing the other directly, carried by dhilipsiva, with the reply addressed directly back. **Phase 3, chapter passes, chapter 1 through 14 in order:** one at a time, read whole, fix, verify, commit, move on; the per-chapter bullets are a floor for that pass, not its scope, and a chapter with no bullets still gets a pass. Everything after phase 3 is cross-cutting (constitution work no chapter owns, harness, data, research brief, legacy harvest) — work it when a chapter pass reaches into it. Delete a bullet when it fully lands; update it if only partly done. book-2's items live in `book-2/TODO.md`; don't work them while book-1 is active.
...
- **Care walks through two existing doors, and chapter 10 says so**, decided 2026-08-02,
  ...
  **pin-file caregiver pair** (chapter 10's suite; fresh names, one route each so each door is
  ...
  ruling and tracked separately in Phase 2: **neither door carries a maturity test** — a child
  who works or teaches mints today (both verified), beside a design that refused student
  minting partly as paying a child. That question is open; do not resolve it by reflexively
  guarding a door until it is ruled.
